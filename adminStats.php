      ...
      <table class="table table-bordered table-striped">
          ...
          <?php
					...
					?>
							...
							<?php $reponse2 = $bdd->query("SELECT SUM(prix) AS annuel FROM transac WHERE date_achat BETWEEN to_timestamp(to_char(Now()::timestamptz, 'YYYY')||'-01-01', 'YYYY-MM-DD') AND Now() AND nom_park = '$park'");
						$reponse3 = $bdd->query("SELECT SUM(prix) AS mensuel FROM transac WHERE date_achat BETWEEN to_timestamp(to_char(Now()::timestamptz, 'YYYY-MM')||'-01', 'YYYY-MM-DD') AND Now() AND nom_park='$park'"); 
							$donnees2 = $reponse2->fetch();
							$donnees3 = $reponse3->fetch();
							?>
							
        						<td><?php echo $donnees2['mensuel']; ?>€</td>
        						<td><?php echo $donnees3['annuel']; ?>€</td>
      						</tr>
					<?php
						}
